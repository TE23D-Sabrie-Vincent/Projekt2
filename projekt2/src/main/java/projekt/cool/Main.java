...
public class Main {
    /*
     * Vad metoden gör: Programmets huvudmetod som driver meny-loopen. Den tar emot
     * inmatning
     * från användaren och anropar rätt funktioner i LibraryManager. Metoden
     * innehåller även
     * felhantering (try-catch) för att förhindra krascher vid inmatning av
    ...
    public static void main(String[] args) {
        ...
        // variabeln kör (boolean) kontrollerar om huvudloopen ska fortsätta eller
        // avslutas.
        boolean kör = true;
        while (kör) {
            ...
            System.out.println("7. Hantera kunder (Ta bort eller lägga till)");
            System.out.println("8. Hämta in JSON data (böcker och Magazines osv)");
            System.out.println("9. Avslut Koden");
            ...
            if (val.equals("1")) {
                ...
                if (manager.getBooks().isEmpty())
                    System.out.println("Biblioteket är tomt.");
                ...
                System.out.println(manager.removeBook(scanner.nextLine()) ? "Feedback: Boken borttagen."
                        : "Feedback: Hittade inte ID.");
            }
                ...
                if (manager.getSortedUsers().isEmpty())
                    System.out.println("Inga kunder registrerade (datan har inte laddats ännu).");
                ...
                System.out.println("\n1. Skapa ny kund till server\n2. Ta bort kund från server\n3. Spärra en kund");
                String kundVal = scanner.nextLine();
                if (kundVal.equals("1")) {
                    System.out.print("ID: ");
                    ...
                    manager.addUser(new User(kid, kname, kemail));
                    System.out.println("Kund uppladdad till servern!");
                }

                else if (kundVal.equals("2")) {
                    System.out.print("Ange e-post för kund som ska tas bort: ");
                    System.out.println(manager.removeUserByEmail(scanner.nextLine()) ? "Kund borttagen från server."
                            : "Kunden hittades inte.");
                }

                else if (kundVal.equals("3")) {
                    System.out.print("Ange unikt Spärr-ID: ");
                    String sid = scanner.nextLine();
                    System.out.print("Ange kundens ID som ska spärras: ");
                    String suid = scanner.nextLine();
                    manager.suspendUser(new SuspendedUser(sid, suid));
                    System.out.println("Kunden har spärrats på servern!");
                }
            }

           else if (val.equals("8")) {
                System.out.println("\n--- Synkroniserar med servern... ---");

                manager.fetchDataFromServer();

                System.out.println("\n--- Böcker ---");
                if (manager.getBooks().isEmpty()) {
                    System.out.println("Biblioteket är tomt på servern (eller så misslyckades anslutningen).");
                } else {
                    for (Book b : manager.getBooks()) {
                        System.out.println(b);
                    }
